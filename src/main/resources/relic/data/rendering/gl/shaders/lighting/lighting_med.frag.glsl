...
uniform DirLight activeDirectionalLight;
uniform Fog fog;
...
void main()
{
    ...
    vec4 diffuseSpecularComp = calcDirLight(diffuse, specular, reflectance, activeDirectionalLight, view_pos, normal);
    ...
    if (fog.activeFog == 1) {
        fragColor = calcFog(world_pos.xyz, fragColor, fog, ambientLight.color, activeDirectionalLight);
    }
}