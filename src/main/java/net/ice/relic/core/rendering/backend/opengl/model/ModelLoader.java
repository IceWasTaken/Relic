...
//    }
    ...
    public Model loadModel(String modelId, Resource resource, boolean animation) {
        return loadModel(modelId, resource, textureCache, materialCache, modelCache, aiProcess_GenSmoothNormals | aiProcess_JoinIdenticalVertices |
                aiProcess_Triangulate | aiProcess_FixInfacingNormals | aiProcess_CalcTangentSpace | aiProcess_LimitBoneWeights |
                aiProcess_GenBoundingBoxes | (animation ? 0 : aiProcess_PreTransformVertices));
    }

    public Model loadModel(String modelId, Resource resource, TextureCache textureCache, MaterialCache materialCache, ModelCache cache, int flags) {
        AIScene aiScene = aiImportFileFromMemory(resource.load(), flags, (String) null);
        if (aiScene == null) {
        ...
        for (int i = 0; i < numMaterials; i++) {
            ...
            Material material = processMaterial(aiScene, aiMaterial, resource.getPath(), textureCache);
...
//    }
    ...
    private static MeshData processMesh(AIMesh aiMesh, List<Bone> boneList) {
        ...
        AnimMeshData animMeshData = new AnimMeshData(new float[]{}, new int[]{});
