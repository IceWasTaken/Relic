...
public class ShadowBuffer {
	...
	public ShadowBuffer() {
		textureArray = glCreateTextures(GL_TEXTURE_2D_ARRAY);

		glTextureStorage3D(textureArray, 1, GL_RG32F, 4096, 4096, 3);
		...
		this.depthTextureArray = glCreateTextures(GL_TEXTURE_2D_ARRAY);

		glTextureStorage3D(depthTextureArray, 1, GL_DEPTH_COMPONENT32F, 4096, 4096, 3);
		glTextureParameteri(depthTextureArray, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTextureParameteri(depthTextureArray, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glTextureParameteri(depthTextureArray, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
		glTextureParameteri(depthTextureArray, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);

		this.handle = glCreateFramebuffers();
		glNamedFramebufferTexture(handle, GL_COLOR_ATTACHMENT0, textureArray, 0);
		glNamedFramebufferTexture(handle, GL_DEPTH_ATTACHMENT, depthTextureArray, 0);
		glNamedFramebufferDrawBuffers(handle, GL_COLOR_ATTACHMENT0);
	...
	public void bindTextureArray(int textureUnit) {
		glBindTextureUnit(textureUnit, textureArray);
	}
