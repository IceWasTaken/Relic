...
void main() {
    ...
    // artifacts).
            ...
            outDownsample = groups[0] + groups[1] + groups[2] + groups[3] + groups[4];
            outDownsample = max(outDownsample, 0.0001f);
            break;
        default:
            outDownsample = e * 0.125;                // ok
            outDownsample += (a+c+g+i)*0.03125;     // ok
            outDownsample += (b+d+f+h)*0.0625;      // ok
            outDownsample += (j+k+l+m)*0.125;       // ok
            break;

        downsample = vec4(outDownsample, 1);
    }
}