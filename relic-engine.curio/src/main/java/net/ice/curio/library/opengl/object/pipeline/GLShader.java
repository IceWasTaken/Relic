...
public class GLShader extends Shader {
	...
	private GLShader(String path, ShaderType type) {
		super(type);

		this.shaderHandle = glCreateShader(getType());

		glShaderSource(shaderHandle, loadFile(path));
		glCompileShader(getType());
	}

	public static List<GLShader> loadProgram(String programPath) {
